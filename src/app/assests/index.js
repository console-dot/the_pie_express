import logo from "./thePieExpress-logo.png";
import firstImage from "./bg_1.jpg";
import SecondImage from "./bg_3.jpg";
import bg2 from "./bg_2.jpg";
import imageSlider1 from "./bg_1.png";
import imageSlider2 from "./bg_2.png";
import about from "./about.jpg";
import pizza1 from "./pizza-1.jpg";
import pizza2 from "./pizza-2.jpg";
import pizza3 from "./pizza-3.jpg";
import pizza4 from "./pizza-4.jpg";
import pizza5 from "./pizza-5.jpg";
import pizza6 from "./pizza-6.jpg";
import pizza7 from "./pizza-7.jpg";
import pizza8 from "./pizza-8.jpg";
import image1 from "./image_1.jpg";
import image2 from "./image_2.jpg";
import image3 from "./image_3.jpg";
import mapImage from "./pie-express-map.png";


export {
  logo,
  firstImage,
  SecondImage,
  bg2,
  imageSlider1,
  imageSlider2,
  about,
  pizza1,
  pizza2,
  pizza3,
  pizza4,
  pizza5,
  pizza6,
  pizza7,
  pizza8,
  image1,
  image2,
  image3,
  mapImage
};
